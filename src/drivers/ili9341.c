...
// =====================================================
// 🔥 WICHTIG:
// Diese funktionen haben KEIN CS mehr!
// CS wird jetzt außerhalb kontrolliert,
// damit command + data als EIN block gesendet werden
// =====================================================
...
static void ili_set_window(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
    CS_LOW(); //EIN zusammenhängender SPI block
...
void ili9341_init(void) {
    ...
    ili_cmd(0x3A); 
    ili_data8(0x55); //erwarte pixel format RGB565

    ili_cmd(0x36); 
    ili_data8(0x48); //setz orientierung und RBG farbreihe
...
void ili9341_fill_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
    ...
    for (uint32_t i = 0; i < total; i++) {
         // jeden pixel mit hi und lo byte befüllen
        spi_transfer(hi); 
        spi_transfer(lo);
    }
